...
def overlay_videos(background_video_memmap, foreground_video_memmap, output_video_memmap, height, width, channels=3):
    ...
    background_frames = np.memmap(background_video_memmap, dtype='uint8', mode='r', shape=(background_num_frames, height, width, channels))
    foreground_frames = np.memmap(foreground_video_memmap, dtype='uint8', mode='r', shape=(foreground_num_frames, height, width, channels))
    ...
    for i in range(num_frames):
        if i < background_num_frames and i < foreground_num_frames:
            ...
            # Apply the masks
            background_frame_masked = cv2.bitwise_and(background_frame, background_frame, mask=background_mask)
            foreground_frame_masked = cv2.bitwise_and(foreground_frame, foreground_frame, mask=foreground_mask)
